"""
Django settings for aindexweb project.
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'context_processors': [
                ...
                'aindexweb.context_processors.site',
            ],
        },
    },
]

WSGI_APPLICATION = 'aindexweb.wsgi.application'
...
SOCIALACCOUNT_PROVIDERS = {
    'google': {
        'SCOPE': env.list('SOCIALACCOUNT_GOOGLE_SCOPE', default=[
            'profile',
            'email',
        ]),
        'AUTH_PARAMS': {
...
STORAGES = {
    ...
    'documents': {
        'BACKEND': env('DOCUMENTS_FILE_STORAGE', default='django.core.files.storage.FileSystemStorage'),
        'OPTIONS': {}
    },
    'decks': {
...
if STORAGES['documents']['BACKEND'] == 'storages.backends.gcloud.GoogleCloudStorage':
    STORAGES['documents']['OPTIONS']['bucket_name'] = env('DOCUMENTS_GS_BUCKET_NAME')
    STORAGES['documents']['OPTIONS']['location'] = env('DOCUMENTS_GS_LOCATION', default='')
    STORAGES['documents']['OPTIONS']['default_acl'] = env('DOCUMENTS_GS_DEFAULT_ACL', default=None)
    STORAGES['documents']['OPTIONS']['querystring_auth'] = env.bool('DOCUMENTS_GS_QUERYSTRING_AUTH', default=True)

    _DOCUMENTS_GS_EXPIRATION = env.int('DOCUMENTS_GS_EXPIRATION', default=86400)
    STORAGES['documents']['OPTIONS']['expiration'] = timedelta(seconds=_DOCUMENTS_GS_EXPIRATION)
...
SITE_NAME = env('SITE_NAME', default='AINdex')
