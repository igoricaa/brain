...
class MissedDealResource(resources.ModelResource):
    ...
    values_mapping = {
        ...
        'revenue_range': {
            '': None,
            **{v: as_decimal_range(k[0], k[1]) for k, v in REVENUE_RANGE_CHOICES.items()}
        },
        'valuation_range': {
            '': None,
            **{v: as_decimal_range(k[0], k[1]) for k, v in REVENUE_RANGE_CHOICES.items()}
    ...
    def parse_diversity_spotlight(self, row):
        ...
        row['has_meo_on_founders'] = any(x in diversity_spotlight
                                         for x in ['middle eastern', 'north african', 'native', 'indigenous'])
