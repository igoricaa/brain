...
export const useFileManagement = () => {
    ...
    const getDealFiles = useCallback(
        async (dealUuid: string, filters?: FileFilters): Promise<PaginatedResponse<DealFile>> => {
            setIsLoading(true);
            setError(null);
    ...
    const uploadDealFile = useCallback(
        async (
            dealUuid: string,
    ...
    const updateDealFile = useCallback(
        async (fileUuid: string, data: UpdateFileRequest): Promise<DealFile> => {
            setIsLoading(true);
            setError(null);
    ...
    const getLibraryFiles = useCallback(
        async (filters?: FileFilters): Promise<PaginatedResponse<LibraryFile>> => {
            setIsLoading(true);
            setError(null);
    ...
    const updateLibraryFile = useCallback(
        async (
            fileUuid: string,
    ...
    const bulkDeleteFiles = useCallback(
        async (fileUuids: string[], mode: 'deal' | 'library'): Promise<void> => {
            setIsLoading(true);
            setError(null);
    ...
    const bulkReprocessFiles = useCallback(
        async (fileUuids: string[], mode: 'deal' | 'library'): Promise<void> => {
            setIsLoading(true);
            setError(null);
    ...
    const downloadFile = useCallback(
        async (fileUuid: string, mode: 'deal' | 'library'): Promise<void> => {
            try {
                const endpoint =
                    mode === 'deal'
    ...
    const subscribeToFileUpdates = useCallback(
        (fileUuids: string[], onUpdate: (fileUuid: string, status: string) => void) => {
