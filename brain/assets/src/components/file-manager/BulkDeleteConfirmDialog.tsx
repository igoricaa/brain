...
export default function BulkDeleteConfirmDialog({
  ...
  const hasCompletedFiles = selectedFiles.some(
        ...
        <AlertDialogHeader>
          ...
          <AlertDialogDescription asChild>
            <div className="space-y-3">
              <p>
                Are you sure you want to delete these {selectedFiles.length} file{selectedFiles.length === 1 ? '' : 's'}? 
                This action cannot be undone.
              </p>
              
              {mode === 'deal' && (
                <p className="text-sm">
                  <strong>Note:</strong> These files will be permanently removed from this deal.
                </p>
              )}
              
              {mode === 'library' && (
                <p className="text-sm">
                  <strong>Note:</strong> These files will be permanently removed from the knowledge base.
                </p>
              )}
              ...
              {hasProcessingFiles && (
                <div className="bg-yellow-50 shadow-sm rounded-lg p-3">
                  ...
                  <p className="text-sm text-yellow-700 mt-1">
                    Some files are currently being processed. Deleting them will stop the processing.
                  </p>
                </div>
              )}
              ...
              {hasCompletedFiles && (
                <div className="bg-red-50 shadow-sm rounded-lg p-3">
                  ...
                  <p className="text-sm text-red-700 mt-1">
                    Some files have been successfully processed. All extracted data and analysis will be permanently lost.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="space-y-4">
              ...
              <div key={status} className="space-y-2">
                <div className="flex items-center gap-2">
                  <Badge 
                    ...
                    className={
                      status === 'completed' ? 'bg-green-100 text-green-800' : undefined
                    }
                  >
                ...
                <div className="bg-muted rounded-lg p-3 space-y-1 max-h-32 overflow-y-auto">
                  {files.slice(0, 10).map((file) => (
                    <div key={file.uuid} className="flex items-center gap-2 text-sm">
            ...
            <div className="bg-red-50 shadow-sm rounded-lg p-3">
              ...
              <ul className="text-sm text-red-700 space-y-1">
                <li>• {selectedFiles.length} file{selectedFiles.length === 1 ? '' : 's'} will be permanently deleted</li>
                {hasCompletedFiles && (
                  <li>• All processed data and extracted information will be lost</li>
                )}
                {hasProcessingFiles && (
