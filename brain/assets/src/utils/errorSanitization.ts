 ...
 */
const sanitizeErrorObject = (error: Error, context?: string, timestamp: number = Date.now()): SanitizedError => {
 ...
 */
const sanitizeStringError = (error: string, context?: string, timestamp: number = Date.now()): SanitizedError => {
 ...
 */
const sanitizeObjectError = (error: Record<string, any>, context?: string, timestamp: number = Date.now()): SanitizedError => {
...
const categorizeErrorMessage = (message: string): ErrorType => {
  ...
  if (lowerMessage.includes('quota') || lowerMessage.includes('storage space') || lowerMessage.includes('localstorage')) {
    return 'quota';
  }
  
  if (lowerMessage.includes('file') || lowerMessage.includes('upload') || lowerMessage.includes('conversion')) {
    return 'file';
  }
  
  if (lowerMessage.includes('unauthorized') || lowerMessage.includes('forbidden') || lowerMessage.includes('auth')) {
    return 'auth';
  }
