...
const initIndexedDB = (): Promise<IDBDatabase> => {
    return new Promise((resolve, reject) => {
        ...
        request.onerror = () => {
            const error: FileOperationError = new Error(
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                ...
                if (prefix) {
                    ...
                    console.log(
                        `🔍 Filtered keys with prefix '${prefix}': ${filteredKeys.length}`,
                        filteredKeys,
                    );
                } else {
                    filteredKeys = stringKeys;
                    console.log(
                        `🔍 No prefix filter, returning all string keys: ${filteredKeys.length}`,
                    );
                }

                console.log(
                    `✅ getAllKeysFromIndexedDB returning ${filteredKeys.length} keys:`,
                    filteredKeys,
                );
...
const compressData = (data: Uint8Array): Uint8Array => {
    try {
        ...
        const compressionError: FileOperationError = new Error(
...
const decompressData = (compressedData: Uint8Array): Uint8Array => {
    try {
        ...
        const decompressionError: FileOperationError = new Error(
...
const base64ToUint8Array = (base64: string): Uint8Array => {
    ...
    try {
        ...
        const conversionError: FileOperationError = new Error(
 ...
 */
export const fileToBase64Streaming = async (
    file: File,
    ...
    if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
        const error: FileOperationError = new Error(
            `File too large: ${Math.round(file.size / 1024 / 1024)}MB exceeds ${MAX_FILE_SIZE_MB}MB limit`,
        ) as FileOperationError;
    ...
    const shouldCompress =
        options.compress !== false && (file.size > 1024 * 1024 || options.compress === true);
    ...
    try {
        ...
        const fileError: FileOperationError = new Error(
            `File conversion failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ) as FileOperationError;
 ...
 */
export const base64ToFileStreaming = (
    base64Data: string,
    filename: string,
    ...
    try {
        ...
        try {
            ...
            if (parsedData.format === 'compressed') {
                ...
                console.log(
                    `Decompressed file: ${filename} (${formatFileSize(parsedData.originalSize)})`,
                );
        ...
        if (fileData.length > MAX_FILE_SIZE_MB * 1024 * 1024) {
            const error: FileOperationError = new Error(
                `Decompressed file too large: ${Math.round(fileData.length / 1024 / 1024)}MB exceeds ${MAX_FILE_SIZE_MB}MB limit`,
            ) as FileOperationError;
            error.code = 'FILE_TOO_LARGE';
            throw error;
        }
        ...
        console.log(
            `File reconstructed: ${filename} (${formatFileSize(file.size)}, ${actualMimeType})`,
        );
        return file;
    } catch (error) {
        const fileError: FileOperationError = new Error(
            `Base64 to File conversion failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ) as FileOperationError;
        fileError.code = 'CONVERSION_FAILED';
        fileError.originalError = error instanceof Error ? error : undefined;
        throw fileError;
 ...
 */
export const enhancedStorageSet = async (
    key: string,
    value: string,
    options: { forceIndexedDB?: boolean; maxChunkSize?: number } = {},
    ...
    console.log(
        `💾 enhancedStorageSet called with key: ${key}, size: ${formatFileSize(originalSize)}, forceIndexedDB: ${forceIndexedDB}`,
    );
    ...
    try {
        ...
        if (compressedBase64.length < value.length * 0.9) {
            ...
            console.log(
                `Compressed storage data: ${formatFileSize(originalSize)} → ${formatFileSize(compressedSize)} (${Math.round((1 - compressedSize / originalSize) * 100)}% reduction)`,
            );
        }
    } catch (error) {
    ...
    console.log(
        `🤔 Storage strategy decision: finalSize=${formatFileSize(finalSize)}, threshold=${formatFileSize(LARGE_FILE_THRESHOLD)}, forceIndexedDB=${forceIndexedDB}`,
    );
 ...
 */
export const safeLocalStorageSet = async (
    key: string,
    value: string,
    originalSize?: number,
    compressedSize?: number,
    ...
    if (!hasSpace) {
        const quota = await getStorageQuota();
        const error: FileOperationError = new Error(
            `Not enough storage space. Need ${Math.round(valueSize / 1024)}KB, available ${Math.round(quota.availableBytes / 1024)}KB`,
        ) as FileOperationError;
    ...
    try {
        ...
        return {
            ...
            compressionRatio:
                originalSize && originalSize > 0 ? (compressedSize || valueSize) / originalSize : 1,
            storageMethod: 'localStorage',
        };
    } catch (error) {
        // Handle quota exceeded or other storage errors
        if (error instanceof DOMException && error.code === 22) {
            const quotaError: FileOperationError = new Error(
        ...
        const storageError: FileOperationError = new Error(
...
export const validateFileIntegrity = async (
    data: string,
): Promise<{ isValid: boolean; repaired?: string; error?: string }> => {
    try {
        ...
        try {
            ...
            return {
                isValid: false,
                error: `Data parsing failed: ${parseError}, Base64 validation failed: ${base64Error}`,
            };
        }
    }
};
