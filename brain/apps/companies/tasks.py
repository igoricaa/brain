...
@shared_task()
def pull_company_affinity_info(pk):
    """Pull and save company info from Affinity."""

    start_time = time.perf_counter()

    company_model = apps.get_registered_model('companies', 'Company')
    company = company_model.objects.get(pk=pk)
    result = company.pull_affinity_info()
...
def save_company_image_from_url(pk, url):
    ...
    return {
        'execution_time': end_time - start_time,
        'parameters': {'pk': pk, 'url': url},
        'saved': saved,
    }


@shared_task()
def pull_company_founder_openai_attrs(pk):
    """Extract and save additional company founder attributes."""

    start_time = time.perf_counter()

    founding_model = apps.get_registered_model('companies', 'Founding')
    founding = founding_model.objects.get(pk=pk)
    result_attributes = founding.pull_openai_attrs()
