...
from ..storage import company_image_path
from ..tasks import (
    pull_company_affinity_info,
    pull_company_clinical_studies,
    pull_company_grants,
    pull_company_openai_attrs,
    pull_company_patent_applications,
    save_company_image_from_url,
)
from .base import (
    AcquisitionTerms,
    AcquisitionType,
...
class Company(models.Model):
    ...
    affinity_id = models.IntegerField(_('affinity organization ID'), null=True, blank=True)
    ...
    image = models.ImageField(_('image'), upload_to=company_image_path, blank=True, null=True)

    image_md = ImageSpecField(source='image', processors=[ResizeToFit(512)], format='PNG', options={'quality': 90})
    image_sm = ImageSpecField(source='image', processors=[ResizeToFit(256)], format='PNG', options={'quality': 90})
    image_xxs = ImageSpecField(source='image', processors=[ResizeToFit(64)], format='PNG', options={'quality': 90})
    ...
    def pull_patent_applications(self):
        ...
        for result in results['patentFileWrapperDataBag']:
            ...
            update_attrs = {
                ...
                for field_name, field_value in patent_attrs.items()
    ...
    def pull_clinical_studies(self):
        ...
        for study in data['studies']:
            ...
            collaborators_names = [
                collaborator.get('name', '') for collaborator in sponsors_collaborators.get('collaborators', [])
            ]
    ...
    def pull_affinity_info(self):
        info = self.get_affinity_info()
        if not info:
            return None

        update_fields = ['affinity_id']
        self.affinity_id = info['id']

        domain = info['domain']
        if domain and not self.website:
    ...
    def pull_attrs(self):
        _tasks = task_group(
            pull_company_affinity_info.si(pk=self.pk),
    ...
    def get_affinity_info(self):
        ...
        orgs = affinity.search_organizations(term=self.name)
        if orgs['organizations']:
            return orgs['organizations'][0]
